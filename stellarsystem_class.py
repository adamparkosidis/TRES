...
class StellarSystem_Class:
    ...
    def __init__(
        ...
        self.tend = args["tend"]
        self.tinit = args["tinit"]
        self.previous_time = 0.0 | units.yr
        self.previous_dt = 0.0 | units.yr
        self.instantaneous_evolution = False
        if args["tinit"] > 0 | units.Myr:
            self.instantaneous_evolution = True  # no secular

        self.maximum_radius_change_factor = args["maximum_radius_change_factor"]
        self.fixed_timestep = -1 | units.Myr

        self.file_name = args["file_name"]
        self.file_type = args["file_type"]
        self.which_common_envelope = args["which_common_envelope"]
        self.include_CHE = args["include_CHE"]
        ...
        self.SN_kick_distr = args["SN_kick_distr"]
        self.impulse_kick_for_black_holes = args["impulse_kick_for_black_holes"]
        self.fallback_kick_for_black_holes = args["fallback_kick_for_black_holes"]
        self.max_CPU_time = args["max_CPU_time"]

        self.triple = bins[-1]
        self.triple.time = 0.0 | units.yr
        self.triple.relative_inclination = args["relative_inclination"]
        self.triple.is_star = False  # maybe not necessary?
        self.triple.dynamical_instability = False
        self.triple.number = args["number"]
        self.triple.error_flag_secular = 0
        self.triple.CPU_time = 0.0
        self.triple.delta_e_in = 0.0 
        self.triple.max_delta_e = 0.0
        ...
        self.initialize_secular(secular_code, self.stop_at_semisecular_regime, self.stop_at_dynamical_instability)

        self.check_RLOF()
        if self.has_tertiary_donor() and (
            self.stop_at_outer_mass_transfer or self.stop_at_mass_transfer or self.stop_at_init_mass_transfer
        ):
        ...
        if (self.has_donor() or self.has_OLOF_donor()) and (
            self.stop_at_mass_transfer or self.stop_at_init_mass_transfer
        ):
            self.mass_transfer_at_initialisation = True
            if self.is_binary(self.triple):
                bin = self.triple
    ...
    def initialize_secular(self, secular_code, stop_at_semisecular_regime, stop_at_dynamical_instability):
...
#                stellar_system.spin_angular_frequency = corotating_spin_angular_frequency_binary(
#                        stellar_system.parent.semimajor_axis,
#                        self.get_mass(stellar_system.parent.child1),
#                        self.get_mass(stellar_system.parent.child2),
#                    )
    ...
    # -------
    def refresh_memory_orbits(self, copy):
        if self.triple.is_star:
            self.triple.spin_angular_frequency =copy.spin_angular_frequency
        else: 
             self.triple.semimajor_axis = copy.semimajor_axis
             self.triple.eccentricity = copy.eccentricity
             self.triple.argument_of_pericenter = copy.argument_of_pericenter
             self.triple.longitude_of_ascending_node = copy.longitude_of_ascending_node
             
             if self.triple.child1.is_star():
                self.triple.child1.spin_angular_frequency = copy.child1.spin_angular_frequency
             else:
                 self.triple.child1.semimajor_axis = copy.child1.semimajor_axis
                 self.triple.child1.eccentricity = copy.child1.eccentricity
                 self.triple.child1.argument_of_pericenter = copy.child1.argument_of_pericenter
                 self.triple.child1.longitude_of_ascending_node = copy.child1.longitude_of_ascending_node
                 
                 if self.triple.child1.child1.is_star() and self.triple.child1.child2.is_star():
                    self.triple.child1.child1.spin_angular_frequency = copy.child1.child1.spin_angular_frequency
                    self.triple.child1.child2.spin_angular_frequency = copy.child1.child2.spin_angular_frequency
                 else: 
                    sys.exit('error in refresh memory orbits: quadruple structure')
                
                    
             if self.triple.child2.is_star():
                self.triple.child2.spin_angular_frequency = copy.child2.spin_angular_frequency
             else:
                 self.triple.child2.semimajor_axis = copy.child2.semimajor_axis
                 self.triple.child2.eccentricity = copy.child2.eccentricity
                 self.triple.child2.argument_of_pericenter = copy.child2.argument_of_pericenter
                 self.triple.child2.longitude_of_ascending_node = copy.child2.longitude_of_ascending_node
                
                 if self.triple.child2.child1.is_star() and self.triple.child2.child2.is_star():
                    self.triple.child2.child1.spin_angular_frequency = copy.child2.child1.spin_angular_frequency
                    self.triple.child2.child2.spin_angular_frequency = copy.child2.child2.spin_angular_frequency
                 else: 
                    sys.exit('error in refresh memory orbits: quadruple structure')
   


    def refresh_memory_stars(self, stellar_system=None):
        if stellar_system == None:
            stellar_system = self.triple
        ...
        if stellar_system.is_star:
            ...
            self.refresh_memory_stars(stellar_system.child1)
            self.refresh_memory_stars(stellar_system.child2)
            stellar_system.mass = stellar_system.previous_mass
    ...
    def update_time_derivative_of_radius(self, stellar_system=None):
        ...
        if stellar_system.is_star:
            if self.triple.time == quantities.zero: # initialization
                stellar_system.time_derivative_of_radius = 0.0 | units.RSun / units.yr
            else: 
                stellar_system.time_derivative_of_radius = (
                    stellar_system.radius - stellar_system.previous_radius) / time_step
        else:
    ...
    def update_time_derivative_of_moment_of_inertia(self, stellar_system=None):
        ...
        if stellar_system.is_star:
            if self.triple.time == quantities.zero: # initialization
                stellar_system.time_derivative_of_moment_of_inertia = 0.0 | units.MSun * units.RSun**2 / units.yr
            else: 
                stellar_system.time_derivative_of_moment_of_inertia = (
                    stellar_system.moment_of_inertia_of_star - stellar_system.previous_moment_of_inertia_of_star
                     ) / time_step
        else:
    ...
    def update_stellar_parameters(self, stellar_system=None):
        ...
        if stellar_system.is_star:
            ...
            if stellar_system.convective_envelope_radius == 0 | units.RSun:
                stellar_system.convective_envelope_mass = 1.0e-10 | units.MSun
                stellar_system.convective_envelope_radius = 1.0e-10 | units.RSun
    ...
    # function below doesn't work well, as it uses bin_types that are set later
    # -> use has_tertiary_donor if a mass transfer in the outer binary of the
    # triple is currently taking place, not if a mass transfer has happened in
    # the past
    ...
    def has_stellar_type_changed_into_SN_remnant(self, stellar_system=None):
        ...
        if stellar_system.is_star:
            ...
            if self.has_stellar_type_changed_into_SN_remnant(
                stellar_system.child1
            ) or self.has_stellar_type_changed_into_SN_remnant(stellar_system.child2):
                return True
    ...
    def has_stellar_type_changed(self, stellar_system=None):
        ...
        if stellar_system.is_star:
            ...
            if self.has_stellar_type_changed(stellar_system.child1) or self.has_stellar_type_changed(
    ...
    def apsidal_motion_constant(self, star):
        ...
        if star.stellar_type in [13] | units.stellar_type:  # ns
            ...
            star.stellar_type in [0, 2, 3, 4, 5, 6, 8, 9, 17] | units.stellar_type
        ):  # low-mass ms, hg, gb, cheb, agb, he-g, pre-ms
            # based on Brooke & Olle 1955, for n=3 polytrope
            #            return 0.143
    ...
    def octupole_parameter(self):
        if self.is_triple():
            ...
            return (
                (self.get_mass(bin.child1) - self.get_mass(bin.child2))
                / self.get_mass(bin)
                * bin.semimajor_axis
    ...
    def get_kozai_type(self): #needs adj for quad
        if self.is_triple():
    ...
    def get_minimum_part_dt_mt(self, stellar_system=None):
        if stellar_system == None:
            stellar_system = self.triple
        ...
        if stellar_system.is_star:
            ...
            p1 = self.get_minimum_part_dt_mt(stellar_system.child1)
    ...
    def set_part_dt_mt(self, value, stellar_system=None):
        ...
        if stellar_system.is_star:
            ...
            stellar_system.part_dt_mt = value
            self.set_part_dt_mt(stellar_system.child1)
            self.set_part_dt_mt(stellar_system.child2)
            return 

    def fully_detached_system(self, stellar_system=None):
        if stellar_system == None:
            stellar_system = self.triple
        ...
        if stellar_system.is_star:
            ...
            bt_det1 = self.fully_detached_system(stellar_system.child1)
            bt_det2 = self.fully_detached_system(stellar_system.child2)
            bt_det3 = True
            if stellar_system.bin_type != "detached":
                bt_det3 = False
            return all([bt_det1, bt_det2, bt_det3])

    def get_all_stellar_radii(self, stellar_system=None):
        if stellar_system == None:
            stellar_system = self.triple
        ...
        if stellar_system.is_star:
            ...
            r1 = self.get_all_stellar_radii(stellar_system.child1)
            r2 = self.get_all_stellar_radii(stellar_system.child2)
            return [r1,r2]

    def get_all_stellar_masses(self, stellar_system=None):
        if stellar_system == None:
            stellar_system = self.triple
        ...
        if stellar_system.is_star:
            ...
            m1 = self.get_all_stellar_masses(stellar_system.child1)
            m2 = self.get_all_stellar_masses(stellar_system.child2)
            return [m1,m2]
    ...
    def check_RLOF(self):
        if self.triple.is_star:
            ...
            Rl1 = self.secular_code.give_roche_radius(self.triple, self.triple.child1, self.triple.child2)
            Rl2 = self.secular_code.give_roche_radius(self.triple, self.triple.child2, self.triple.child1)
            
            if REPORT_EVOLUTION:
                print("Roche lobe radii:", Rl1, Rl2)
                print(
                    "Stellar radii:",
                    self.triple.child1.radius,
                    self.triple.child2.radius,
                )
...
#            Rl2 = roche_radius(bin, bin.child2, self)
            Rl_list = self.secular_code.give_roche_radii(self.triple)
            Rl1 = Rl_list[0]
            Rl2 = Rl_list[1]
            if REPORT_EVOLUTION:
                print("Roche lobe radii:", Rl1, Rl2)
...
#            Rl1, Rl2, Rl3 = self.secular_code.give_roche_radii(self.triple)
            ...
            if REPORT_EVOLUTION:
                ...
                print(
                    ...
                    roche_radius(bin, bin.child1, self),
                    roche_radius(bin, bin.child2, self),
                    roche_radius(self.triple, star, self),
                )
                print(
    ...
    def check_OLOF(self):
        if self.triple.is_star:
            ...
            Rl2_1 = L2_radius(self.triple, self.triple.child1, self)
            Rl2_2 = L2_radius(self.triple, self.triple.child2, self)
            if REPORT_EVOLUTION:
    ...
    def check_CHE(self):  # future option: potentially use: che_flag in SeBa
        ...
        if self.triple.is_star:
            ...
            if self.include_CHE and (
                (
                    bin.child1.spin_angular_frequency >= criticial_angular_frequency_CHE(bin.child1.mass, metallicity)
                    and bin.child1.stellar_type <= 1 | units.stellar_type
                )
                or (
                    bin.child2.spin_angular_frequency >= criticial_angular_frequency_CHE(bin.child2.mass, metallicity)
    ...
    def print_binary(self, binary):
        if not binary.is_star:
            ...
            print(
                binary.argument_of_pericenter,
            )
            print(
                binary.longitude_of_ascending_node,
            )
            print(
                binary.mass_transfer_rate,
            )
            #            print(binary.mass_transfer_timescale,)
            print(
                binary.accretion_efficiency_mass_transfer,
            )
            print(
                binary.accretion_efficiency_wind_child1_to_child2,
            )
            print(
                binary.accretion_efficiency_wind_child2_to_child1,
            )
            print(
                binary.specific_AM_loss_mass_transfer,
            )
    ...
    def save_snapshot(self):
        ...
        # del snapshot_triple.particles_set.delta_e_in
        # del snapshot_triple.particles_set.max_delta_e
    ...
    def determine_time_step_wind(self, stellar_system=None):
        ...
        if stellar_system.is_star:
            ...
            if stellar_system.wind_mass_loss_rate * -1.0 > quantities.zero:
                dt = (
                    maximum_wind_mass_loss_factor
                    * (stellar_system.mass - stellar_system.core_mass)
                    / stellar_system.wind_mass_loss_rate
                    * -1.0
                )
            if REPORT_DT:
    ...
    def determine_time_step_radius_change(self, stellar_system=None):
        ...
        if stellar_system.is_star:
            if stellar_system.time_derivative_of_radius == quantities.zero:
                ...
                if (
                    stellar_system.stellar_type > 1 | units.stellar_type
                    and stellar_system.time_derivative_of_radius < stellar_system.previous_time_derivative_of_radius
                ):  # not a MS star and Rdot < Rdot_prev
                    growth_factor = 1.0

                dt = abs(
                    growth_factor
                    * self.maximum_radius_change_factor
                    * stellar_system.radius
                    / stellar_system.time_derivative_of_radius
                )
        ...
        if stellar_system.is_star:
            ...
            if REPORT_DT:
                print(
                    ...
                    stellar_system.mass,
                    stellar_system.parent.mass_transfer_rate,
                )
            return dt
        else:
    ...
    def e_dot_tides(self, star, m_comp, semi, eccentricity):
        e_p2 = eccentricity**2
        l = np.sqrt(1.0 - e_p2)
        f_tides3 = 1.0 + e_p2 * (15.0 / 4.0 + e_p2 * (15.0 / 8.0 + e_p2 * 5.0 / 64.0))
        f_tides4 = 1.0 + e_p2 * (3.0 / 2 + e_p2 / 8.0)

        R_div_a = star.radius / semi
        spin = star.spin_angular_frequency
        k_div_T_tides = tidal_friction_constant(
            star.stellar_type,
            star.mass,
            m_comp,
            semi,
        ...
        e_dot = (
            ...
            * eccentricity
            * (l**-13.0)
            * (f_tides3 - 11.0 / 18.0 * (l**3) * f_tides4 * (spin / n))
        )
...
#            Rl2 = roche_radius(self, self.child2)
            ...
            if star.radius >= Rl3:
                ...
                return abs(
                    time_step_factor_stable_mt * min(bin.child1.mass, bin.child2.mass) / self.triple.mass_transfer_rate
                )
            else:
                de_dt_star = abs(
                    self.e_dot_tides(
    ...
    def determine_time_step(self):
        ...
        if not self.has_donor():
            ...
            time_step = min(
                time_step_radius_change,
                min(time_step_wind, min(min(time_step_stellar_code), time_step_max)),
            )
            if time_step_tides < time_step:
                if REPORT_DT:
                    ...
                    Rl_list = self.secular_code.give_roche_radii(self.triple)
                    print(Rl_list)
                    print(self.get_all_stellar_radii())
        ...
        elif self.has_donor() and self.triple.bin_type == "detached" and self.is_binary():
            ...
            self.check_RLOF() # resetting is_donor
        elif self.has_donor() and self.triple.bin_type == "detached" and self.is_triple():
            if ((self.is_binary(self.triple.child1) and self.triple.child1.bin_type == "detached") or 
                (self.is_binary(self.triple.child2) and self.triple.child2.bin_type == "detached")):
                # find beginning of RLOF carefully
                time_step = time_step_factor_find_RLOF * self.previous_dt
                if REPORT_DT:
                    print("find rlof", time_step_factor_find_RLOF, self.previous_dt)
        ...
        if self.is_binary() and self.triple.bin_type == "detached" and self.triple.previous_bin_type == "stable_mass_transfer":
            corotation_spin = corotating_spin_angular_frequency_binary(
                self.triple.semimajor_axis,
                self.triple.child1.mass,
                self.triple.child2.mass,
            )
            self.triple.child1.spin_angular_frequency = corotation_spin
            self.triple.child2.spin_angular_frequency = corotation_spin
        ...
        elif (self.is_triple() and self.is_binary(self.triple.child1) and self.triple.child1.bin_type == "detached" and 
            self.triple.child1.previous_bin_type == "stable_mass_transfer"):
            corotation_spin = corotating_spin_angular_frequency_binary(
                self.triple.child1.semimajor_axis,
                self.triple.child1.child1.mass,
                self.triple.child1.child2.mass,
            )
            self.triple.child1.child1.spin_angular_frequency = corotation_spin
            self.triple.child1.child2.spin_angular_frequency = corotation_spin

            # if REPORT_DT or REPORT_DEBUG:
            #     print('prev timestep', time_step, previous_time_step)
            previous_time_step = self.triple.time - self.previous_time
            time_step = min(time_step, maximum_time_step_factor_after_stable_mt * previous_time_step)
        elif (self.is_triple() and self.is_binary(self.triple.child2) and self.triple.child2.bin_type == "detached" and 
            self.triple.child2.previous_bin_type == "stable_mass_transfer"):
            corotation_spin = corotating_spin_angular_frequency_binary(
                self.triple.child2.semimajor_axis,
                self.triple.child2.child1.mass,
                self.triple.child2.child2.mass,
            )
    ...
    def random_paczynski_velocity(self, v_disp):
        ...
        max_distr = 4.0 / np.pi
        v_max = 4
        prob = max_distr * np.random.uniform(0.0, 1.0)
        velocity = v_max * np.random.uniform(0.0, 1.0)
        dist_value = max_distr / (1 + velocity**2.0) ** 2.0
    ...
    def adjust_system_after_supernova_kick(self):
        ...
        inner_eccentric_anomaly = optimize.brentq(
            self.anomaly_converter,
            0.0,
            2.0 * np.pi,
            args=(inner_ecc, inner_mean_anomaly),
        )
        inner_true_anomaly = 2.0 * np.arctan2(
            np.sqrt(1 - inner_ecc) * np.cos(inner_eccentric_anomaly / 2.0),
            np.sqrt(1 + inner_ecc) * np.sin(inner_eccentric_anomaly / 2.0),
        )
        ...
        outer_mean_anomaly = np.random.uniform(0.0, 2.0 * np.pi)
        outer_eccentric_anomaly = optimize.brentq(
            self.anomaly_converter,
            0.0,
            2.0 * np.pi,
            args=(outer_ecc, outer_mean_anomaly),
        )
        outer_true_anomaly = 2.0 * np.arctan2(
            np.sqrt(1 - outer_ecc) * np.cos(outer_eccentric_anomaly / 2.0),
        ...
        if REPORT_SN_EVOLUTION:
            ...
            print(
                "inner anomaly:",
                inner_mean_anomaly,
                inner_eccentric_anomaly,
                inner_true_anomaly,
            )
            print(
                "outer anomaly:",
                outer_mean_anomaly,
                outer_eccentric_anomaly,
                outer_true_anomaly,
            )
        ...
        self.channel_to_secular.copy()
        V1, V2, V3 = self.secular_code.compute_effect_of_SN_on_triple(
            vel_kick1,
            vel_kick2,
            vel_kick3,
            dm1,
        ...
        if (
            ...
            or self.triple.eccentricity < 0.0
        ...
        if self.stop_at_SN:
            if REPORT_SN_EVOLUTION:
                ...
                print(self.get_all_stellar_masses())
            return False
        ...
        if stellar_system.is_star:
            ...
            if stopping_condition > 0:
                ...
                if not stellar_system.child1.is_star:  # child1 is a multiple
                    ...
                    sys.exit(
                        "resolve_stellar_interaction: structure stellar system unknown, both children are binaries"
                    )
    ...
    def solve_for_partial_time_step_stable_mass_transfer(self, min_part_dt_mt):
        full_dt = self.triple.time - self.previous_time
        self.secular_code.evolve_model(self.previous_time + full_dt * min_part_dt_mt)
    ...
    def safety_check_time_step(self, stellar_system=None):
        ...
        if stellar_system.is_star:
            ...
            if (
                (abs(dm) > error_dm)
                and not (stellar_system.stellar_type != stellar_system.previous_stellar_type)
                ...
                print(dm, error_dm, stellar_system.mass, stellar_system.previous_mass)
        ...
        if nr_unsuccessfull > 1:
            for i_rec in range(nr_unsuccessfull):
                ...
                if dt_new <= minimum_time_step:
                    triple_number += 0.5
                dr = (
                    star_unsuccessfull[i_rec].radius - star_unsuccessfull[i_rec].previous_radius
    ...
    def check_stopping_conditions_stellar(self, stellar_system=None):
        ...
        if self.is_triple(stellar_system):
            ...
            if (self.has_donor(stellar_system) or self.has_OLOF_donor(stellar_system)) and (
                self.stop_at_mass_transfer
                or (self.stop_at_stable_mass_transfer and stellar_system.is_mt_stable)
                or (self.stop_at_unstable_mass_transfer and not stellar_system.is_mt_stable)
                or (
                ...
                if REPORT_EVOLUTION:
                    ...
                    print(
                        self.stop_at_mass_transfer,
                        self.stop_at_stable_mass_transfer,
                        self.stop_at_unstable_mass_transfer,
                        self.stop_at_eccentric_stable_mass_transfer,
                        self.stop_at_eccentric_unstable_mass_transfer,
                        stellar_system.is_mt_stable,
                    )
    ...
    def check_stopping_conditions(self):
        ...
        if self.stop_at_inner_collision and self.triple.inner_collision == True:
            ...
            elif self.is_triple() and self.is_binary(self.triple.child1):
                self.triple.child1.bin_type = bin_type["collision"]        
            elif self.is_triple() and self.is_binary(self.triple.child2):
                self.triple.child2.bin_type = bin_type["collision"]
            else:
                sys.exit('error in check stopping conditions: quadruple structure')
            if REPORT_EVOLUTION:
        ...
        if self.triple.time - self.secular_code.model_time > numerical_error | units.Myr:
            print(
                ...
                self.triple.time,
                self.secular_code.model_time,
            )
            print(
                self.has_donor(),
                self.secular_code.triples[0].error_flag_secular,
                self.secular_code.triples[0].dynamical_instability,
            )
            return False
    ...
    def calculate_maximum_change_eccentricity(self):
        if self.triple.delta_e_in > self.triple.max_delta_e:
            self.triple.max_delta_e = self.triple.delta_e_in
    ...
    def evolve_model(self, tend):
        ...
        if REPORT_DEBUG or MAKE_PLOTS:
            ...
            Rl_list = self.secular_code.give_roche_radii(self.triple)
            RL1_array.append(Rl_list[0].value_in(units.RSun))
            RL2_array.append(Rl_list[1].value_in(units.RSun))
            delta_e_in_array.append(self.triple.delta_e_in)            
            ...
            if self.is_binary():
                m1_array.append(self.triple.child1.mass)
                m2_array.append(self.triple.child2.mass)
                spin1_array.append(self.triple.child1.spin_angular_frequency.value_in(1.0 / units.Myr))
                spin2_array.append(self.triple.child2.spin_angular_frequency.value_in(1.0 / units.Myr))
                r1_array.append(self.triple.child1.radius.value_in(units.RSun))
                r2_array.append(self.triple.child2.radius.value_in(units.RSun))
                moi1_array.append(self.triple.child1.moment_of_inertia_of_star.value_in(units.RSun**2 * units.MSun))
                moi2_array.append(self.triple.child2.moment_of_inertia_of_star.value_in(units.RSun**2 * units.MSun))
             
            if self.is_triple() and self.is_binary(self.triple.child1):
                RL3_array.append(Rl_list[2].value_in(units.RSun))
        
                e_in_array.append(self.triple.child1.eccentricity)
                a_in_array.append(self.triple.child1.semimajor_axis)
                g_in_array.append(self.triple.child1.argument_of_pericenter)
                o_in_array.append(self.triple.child1.longitude_of_ascending_node)
                
                m1_array.append(self.triple.child1.child1.mass)
                m2_array.append(self.triple.child1.child2.mass)
                m3_array.append(self.triple.child2.mass)
                spin1_array.append(self.triple.child1.child1.spin_angular_frequency.value_in(1.0 / units.Myr))
                spin2_array.append(self.triple.child1.child2.spin_angular_frequency.value_in(1.0 / units.Myr))
                spin3_array.append(self.triple.child2.spin_angular_frequency.value_in(1.0 / units.Myr))
                r1_array.append(self.triple.child1.child1.radius.value_in(units.RSun))
                r2_array.append(self.triple.child1.child2.radius.value_in(units.RSun))
                r3_array.append(self.triple.child2.radius.value_in(units.RSun))
                moi1_array.append(self.triple.child1.child1.moment_of_inertia_of_star.value_in(units.RSun**2 * units.MSun))
                moi2_array.append(self.triple.child1.child2.moment_of_inertia_of_star.value_in(units.RSun**2 * units.MSun))
                moi3_array.append(self.triple.child2.moment_of_inertia_of_star.value_in(units.RSun**2 * units.MSun))
                ...
                spin1_array.append(self.triple.child2.child1.spin_angular_frequency.value_in(1.0 / units.Myr))
                spin2_array.append(self.triple.child2.child2.spin_angular_frequency.value_in(1.0 / units.Myr))
                spin3_array.append(self.triple.child1.spin_angular_frequency.value_in(1.0 / units.Myr))
        ...
        while self.triple.time < self.tend:
            ...
            if self.instantaneous_evolution == False:
                ...
                min_part_dt_mt = self.get_minimum_part_dt_mt()    
                if (
                    min_part_dt_mt < 1
                ):  # inner binary, see function determine_partial_time_step_stable_mass_transfer
                    print("skipping")
                    if self.solve_for_partial_time_step_stable_mass_transfer(min_part_dt_mt) == False:
                        break
                else:
                ...
                if (
                    self.triple.time - self.secular_code.model_time < -1 * numerical_error | units.Myr
                    ...
                    < self.triple.time - max(minimum_time_step, 0.01 * self.determine_time_step_stable_mt())
                ):
                    ...
                    self.refresh_memory_orbits(snapshot_triple)
                    continue
                elif (
                    self.has_donor()
                    and self.fully_detached_system()
